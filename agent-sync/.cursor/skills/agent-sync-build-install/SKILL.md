...
description: Build, test, and install the local `agent-sync` Python CLI from the repository
  checkout using the project's wheel-first packaging workflow. Use when an agent needs
  to create fresh `dist/` artifacts, reinstall the newest local wheel, verify the
  installed CLI, explain why temporary `agent_sync.egg-info/` metadata appears during
  builds, or update project docs about local packaging and installation for `agent-sync/`.
---
