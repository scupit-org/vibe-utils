...
$\theta, \phi$. With $e_y = \pi/2$, $R_y$ swings the body's $X$ axis onto
the world $-Z$ axis (and the body's $Z$ axis onto the world $X$ axis), so
the subsequent $R_x(\theta)$ rotates about an axis that's now world-$-Z$.
Meanwhile $R_z(\phi)$, applied first, rotates about world $Z$. Both end up
generating rotations about the world $Z$-axis, just with opposite signs:
shifting $e_x$ by $\delta$ and $e_z$ by $\delta$ produces equal-and-opposite
contributions, so the net rotation is unchanged. Two parameters are
controlling one degree of freedom --- gimbal lock.
